...
impl MiniV8 {
    ...
    /// Set microtask policy.
    pub fn set_microtasks_policy(&self, policy: v8::MicrotasksPolicy) {
        self.scope(|scope| {
            let isolate: &mut v8::Isolate = scope.as_mut();
            isolate.set_microtasks_policy(policy.into());
        });
    }

    /// Processes the microtasks queue.
